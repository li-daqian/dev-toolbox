...
6. 异常处理、降级路径和静默跳过逻辑必须说明为什么允许继续执行，以及调用方如何感知失败；禁止用注释掩盖吞异常。
7. 注释语言默认跟随文件或项目现有风格；技术名词、协议名、配置键、函数名和错误码保留原文。
8. 不为生成文件、机械映射、样板代码批量添加解释性注释；如需标记生成来源，只保留生成器、输入来源和再生成方式。
