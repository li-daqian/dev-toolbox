...
1. 需要兼容旧版本、旧接口、旧配置、旧数据结构或旧行为前，先判断旧版本是否已经上线、是否有生产数据、外部用户、第三方客户端、公开文档或已部署环境依赖。
2. 如果旧版本尚未上线，且仅存在于本地 git、未合并分支、开发样例或未发布代码中，默认不做向后兼容；优先直接收敛到正确实现，避免保留无意义的兼容层、别名、双协议、迁移脚本或 deprecated 分支。
3. 如果是否需要兼容无法验证，必须显式标注“未验证”，说明需要兼容的假设、验证方式和不兼容修改的风险；高风险时先向用户确认。
4. 确实需要兼容时，应说明兼容对象、兼容期限、迁移路径和删除条件，避免永久保留临时代码。
...
1. 禁止用可读性差、不可维护的弱类型结构冒充业务模型，例如 raw `Map`、`Map<String, Object>`、嵌套 `Map/List`、`dict[str, Any]`、`Record<string, unknown>`、位置语义数组/元组或未命名 JSON blob。
2. 当字段、键、取值范围或层级结构已知时，优先定义有名称的类型：Java 使用 `record`、class、enum、value object、DTO 或 sealed type；其他语言使用对应的 struct、dataclass、interface、type alias、schema 或 typed object。
3. `Map` / `dict` / object bag 只允许用于真实动态键集合，例如 lookup table、cache、headers/query 参数、metadata 扩展点、外部系统原样透传 payload 或 schema 未知的边界输入。
4. 弱类型结构如果确实必要，必须限制在边界层或局部实现内，尽快校验并转换成强类型模型；不得跨 domain、service、API、MCP、WebSocket/SSE 或持久化边界长期传播。
5. 对保留弱类型结构的代码，必须说明动态性的来源、允许的 key/value 约束、调用方如何感知类型错误，以及后续收敛为强类型模型的条件。
6. 公共 API、配置项、数据库实体、消息事件、命令参数和跨模块返回值应暴露稳定、可命名、可搜索、可测试的结构，不用匿名容器隐藏语义。

## 7. 代码注释规范

1. 注释优先解释 `why`，而不是复述 `what`：不要用注释重复代码字面含义；能通过命名、结构或类型表达清楚的内容，优先改代码本身。
2. 以下场景应补充注释：非显而易见的业务规则、兼容旧行为的原因、外部系统限制、并发/事务/缓存/权限/安全边界、复杂算法、反直觉实现、临时 workaround。
3. 注释必须靠近被解释的代码，并随代码修改同步更新；禁止保留过期注释、误导性注释、被注释掉的废弃代码块。
4. `TODO`、`FIXME`、`HACK` 必须包含可追踪编号或明确来源，并说明原因、影响范围和删除条件；没有追踪信息时不要新增。
5. 公共 API、配置项、环境变量、CLI 参数和扩展点应说明语义、默认行为、边界条件和兼容影响；不要只写示例而缺少约束。
6. 方法/函数、方法参数、结构体字段必须有注释说明，至少覆盖语义、单位/格式、可空性、默认值和边界条件；禁止只重复名称或类型。
7. 异常处理、降级路径和静默跳过逻辑必须说明为什么允许继续执行，以及调用方如何感知失败；禁止用注释掩盖吞异常。
8. 注释语言默认跟随文件或项目现有风格；技术名词、协议名、配置键、函数名和错误码保留原文。
9. 不为生成文件、机械映射、样板代码批量添加解释性注释；如需标记生成来源，只保留生成器、输入来源和再生成方式。
...
5. 需要展示架构、流程、调用链、数据流、部署拓扑等核心图时，优先使用 boxed ASCII 图，例如用 `+---+`、`|   |`、箭头和标签表达模块边界与流向；除非用户明确要求 Mermaid、SVG 或其他格式。
EOF
}
